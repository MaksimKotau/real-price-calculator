...
export const getVolumeMinId = (data: VolumeCompareData[]) => {
    ...
    if (data.every(el => Math.abs(getPriceBy100ml(el.unitType, el.count, el.price) - getPriceBy100ml(data[0].unitType, data[0].count, data[0].price)) < 0.01)){
        return undefined
    }
    let indexMin = 0;
    let minId = data[indexMin].id;
    for (let i = 1; i< data.length; i++){
        if (getPriceBy100ml(data[i].unitType, data[i].count, data[i].price) < getPriceBy100ml(data[indexMin].unitType, data[indexMin].count, data[indexMin].price)){
            minId = data[i].id;
            indexMin = i;
        }
    }
...
export const getVolumeMaxId = (data: VolumeCompareData[]) => {
    ...
    for (let i = 1; i< data.length; i++){
        if (getPriceBy100ml(data[i].unitType, data[i].count, data[i].price) > getPriceBy100ml(data[indexMax].unitType, data[indexMax].count, data[indexMax].price)){
            maxId = data[i].id;
            indexMax = i;
        }
    }
