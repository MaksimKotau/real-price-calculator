...
import {
  CountCompareData,
  removeDataFromCountAndSaveToHistory,
  useStore,
} from "../../state/state";
...
const CountTab: React.FC<{}> = () => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "relative" }}>
      ...
      <CountForm isOpen={isFormOpen} handleClose={() => setFormOpen(false)} />
    </div>
  );
};
...
export const renderCountElement = (el: CountCompareData) => {
  return (
    <>
      ...
      <Typography variant='subtitle2'>{`REAL PRICE FOR 1 UNIT: ${getPriceBy1Unit(
        el.count,
        el.price
...
export const getCountMinId = (data: CountCompareData[]) => {
  ...
  if (
    data.every(
      (el) =>
        Math.abs(
          getPriceBy1Unit(el.count, el.price) -
            getPriceBy1Unit(data[0].count, data[0].price)
        ) < 0.001
    )
  ) {
    return undefined;
  ...
  for (let i = 1; i < data.length; i++) {
    if (
      getPriceBy1Unit(data[i].count, data[i].price) <
      getPriceBy1Unit(data[indexMin].count, data[indexMin].price)
    ) {
      minId = data[i].id;
      indexMin = i;
    }
...
export const getCountMaxId = (data: CountCompareData[]) => {
  ...
  for (let i = 1; i < data.length; i++) {
    if (
      getPriceBy1Unit(data[i].count, data[i].price) >
      getPriceBy1Unit(data[indexMax].count, data[indexMax].price)
    ) {
      maxId = data[i].id;
      indexMax = i;
    }
