...
import {
  removeDataFromWeightAndSaveToHistory,
  useStore,
  WeightCompareData,
} from "../../state/state";
...
const WeightTab: React.FC<{}> = () => {
  ...
  const elements: SwipableListElement[] = state.calculation.weight.map(
    (el) => ({
      id: el.id,
  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "relative" }}>
      ...
      <WeightForm isOpen={isFormOpen} handleClose={() => setFormOpen(false)} />
    </div>
  );
};
...
export const renderWeightElement = (el: WeightCompareData) => {
  return (
    <>
      ...
      <Typography variant='subtitle2'>{`REAL PRICE FOR 100g: ${getPriceBy100g(
        el.unitType,
        el.count,
        el.price
...
export const getWeightMinId = (data: WeightCompareData[]) => {
  ...
  for (let i = 1; i < data.length; i++) {
    if (
      getPriceBy100g(data[i].unitType, data[i].count, data[i].price) <
      getPriceBy100g(
